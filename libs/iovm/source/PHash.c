...
unit32_t
crc32(unit32_t hash, const void *buffer, size_t length)
{
	const uint8_t *pointer;

	pointer = buffer;
	hash = hash ^ ~0U; //bitwise not
	while(length--)
		hash = crc32_table[(hash ^ *pointer++) & 0xFF] ^ (hash >> 8);

	return hash ^ ~0U;// Reverse
}
...
crc64(uint64_t crc64, const unsigned char *string, uint64_t length)
{
	uint64_t hash;

	for (hash = 0; hash < length; hash++)
	{
		uint8_t currbyte = string[hash];
		crc64 = crc32_table[(uint8_t)crc64 ^ currbyte] ^ (crc64 >> 8);
	}
